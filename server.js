...
app.post('/api/analyze', authenticateToken, async (req, res) => {
  try {
    ...
    const prompt = `
      ...
      核心指令：
      1. 识别图中有几个餐具/容器，餐具的数量或块数决定了这份食物默认的份数。🚨注意：如果图中有3块牛脊骨，那么 portions 必须是 3，gramsPerPortion 必须是【单块】的重量！坚决不能把多块算成 1 份总重量！
      2. 为每个独立的食物提供名称。🚨名称的括号中不仅要暗示做法，还要简单描述食品每份内部的数量状况，如"油炸黄花鱼(少油，2条/份)", "煎鸡蛋(2个/份)", "西红柿炒鸡蛋(重油)"。
      3. 估算每个食物的"基础份数"(如上所述，图里有几个碗/几块肉就是几份) 和 "每份重量(克)"。
